...
const Sidebar = ({ 
  ...
  <div className="w-64 bg-gray-900 text-white flex flex-col h-screen fixed left-0 top-0 overflow-y-auto print:hidden z-10">
    <div className="p-6 border-b border-gray-800">
      <h2 className="text-2xl font-bold tracking-tight">RetenFácil</h2>
      <p className="text-xs text-gray-400 mt-1">Gestión SENIAT</p>
    </div>
    <nav className="flex-1 p-4 space-y-2">
      <button 
        onClick={() => setRoute(AppRoute.DASHBOARD)}
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition ${currentRoute === AppRoute.DASHBOARD ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
      >
        <span className="material-icons text-lg">dashboard</span>
        Dashboard
      </button>
      <button 
        onClick={() => setRoute(AppRoute.CREATE_RETENTION)}
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition ${currentRoute === AppRoute.CREATE_RETENTION ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
      >
        <span className="material-icons text-lg">post_add</span>
        Nueva Retención
      </button>
      <button 
        onClick={() => setRoute(AppRoute.CREATE_COMPANY)}
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition ${currentRoute === AppRoute.CREATE_COMPANY ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800'}`}
      >
...
const App: React.FC = () => {
  ...
  const fetchVouchers = async () => {
      try {
          // Fetch retentions and join with companies
          ...
          if(data) {
              const mapped: VoucherType[] = data.map((r: any) => ({
                  ...
                  items: r.items // JSONB auto-parsed
              }));
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setUser(session?.user ?? null);
      if (session?.user) {
          if (route === AppRoute.LANDING) setRoute(AppRoute.DASHBOARD);
          fetchProfile(session.user.id);
      } else {
  ...
  const generateVoucher = async () => {
    const company = companies.find(c => c.id === wizCompanyId);
    if (!company || !user) return;

    const voucherNumber = `${new Date().getFullYear()}${String(new Date().getMonth()+1).padStart(2, '0')}000${Math.floor(Math.random()*100)}`;
    const dateStr = new Date().toISOString().split('T')[0];
    const fiscalPeriod = `${new Date().getFullYear()} ${new Date().toLocaleString('es-VE', { month: 'long' })}`;
    ...
    // Save to Supabase
    const { data, error } = await supabase.from('retentions').insert({
        user_id: user.id,
    ...
    if (error) {
        alert("Error al guardar la retención: " + error.message);
        return;
    }
    ...
    // Update view with ID from DB if needed, or just use temp
  ...
  // Group by Fiscal Period for Monthly Summary
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans">
      <Sidebar currentRoute={route} setRoute={setRoute} handleLogout={handleLogout} />
      ...
      <main className="flex-1 ml-64 p-8 print:ml-0 print:p-0">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                         ...
                         <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                                ...
                                <button onClick={() => setRoute(AppRoute.CREATE_RETENTION)} className="text-blue-600 text-sm font-medium hover:underline">
                                    + Nueva
                                </button>
                            </div>
                            <div className="overflow-x-auto">
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-4 text-right">
                                                        <span className="material-icons text-gray-400 text-sm group-hover:text-blue-600">visibility</span>
                                                    </td>
                                                </tr>
                                            );
                                        }) : (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <button onClick={() => setRoute(AppRoute.CREATE_COMPANY)} className="text-blue-600 text-sm font-medium hover:underline">+ Agregar</button>
                            </div>
                            {companies.length > 0 ? (
                                <div className="space-y-3">
                                    {companies.slice(0, 5).map(c => (
                                        <div key={c.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                            <div className="flex items-center gap-3">
                                                {c.logoUrl ? (
                                                    <img src={c.logoUrl} alt="Logo" className="w-8 h-8 rounded object-cover border border-gray-200" />
                                                ) : (
                                                    <div className="w-8 h-8 bg-gray-200 rounded text-xs flex items-center justify-center font-bold text-gray-600">
        ...
        {/* CREATE COMPANY VIEW - REORDERED: RIF, Name, Percentage, Address, Logo */}
        {route === AppRoute.CREATE_COMPANY && (
            <div className="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-gray-200">
                <h2 className="text-2xl font-bold mb-6">Registrar Empresa (Agente)</h2>
                <form onSubmit={handleSaveCompany} className="space-y-5">
                    
                    {/* 1. RIF */}
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">RIF</label>
                        <input required name="rif" type="text" className="w-full border rounded-lg p-2" placeholder="J-12345678-9" />
                    </div>

                    {/* 2. Name */}
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Nombre / Razón Social</label>
                        <input required name="name" type="text" className="w-full border rounded-lg p-2" placeholder="Ej. INVERSIONES ABC C.A." />
                    </div>

                    {/* 3. Percentage */}
                    <div>
                         <label className="block text-sm font-medium text-gray-700 mb-2">Porcentaje de Retención (Contribuyente Especial)</label>
                         <div className="flex gap-4">
                             <label className="flex items-center gap-2 border p-3 rounded-lg cursor-pointer hover:bg-gray-50 w-full">
                                 <input type="radio" name="retentionPercentage" value="75" required defaultChecked className="w-4 h-4 text-blue-600"/>
                                 <div>
                             ...
                             <label className="flex items-center gap-2 border p-3 rounded-lg cursor-pointer hover:bg-gray-50 w-full">
                                 <input type="radio" name="retentionPercentage" value="100" required className="w-4 h-4 text-blue-600"/>
                                 <div>
                    ...
                    <div>
                        ...
                        <textarea required name="address" rows={3} className="w-full border rounded-lg p-2" placeholder="Av. Principal..."></textarea>
                    </div>
                    ...
                    <div className="pt-4 flex justify-end gap-3 border-t border-gray-100 mt-6">
                        <button type="button" onClick={() => setRoute(AppRoute.DASHBOARD)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">Cancelar</button>
                        <button type="submit" disabled={isUploading} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 font-medium">
                            {isUploading ? 'Subiendo...' : 'Guardar Empresa'}
                        </button>
                    </div>
                </form>
            </div>
        ...
        {route === AppRoute.CREATE_RETENTION && (
            <div className="max-w-4xl mx-auto">
                <div className="mb-8 flex items-center justify-between">
                    <h2 className="text-2xl font-bold">Nueva Retención de IVA</h2>
                            ...
                            <div className="space-y-3">
                                {companies.map(c => (
                                    <div 
                                        ...
                                        className="p-4 border rounded-lg hover:border-blue-500 cursor-pointer hover:bg-blue-50 transition flex justify-between items-center group"
                                    >
                                        <div className="flex items-center gap-3">
                                            {c.logoUrl ? (
                    ...
                    <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-200 relative">
                        ...
                        <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-6">
                             ...
                             <div className="relative">
                                 ...
                                 />
                                 <button className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-lg shadow-lg hover:shadow-xl transition flex items-center gap-2">
        ...
        {route === AppRoute.VIEW_RETENTION && currentVoucher && (
            <div className="flex flex-col items-center">
                <div className="w-full max-w-[210mm] flex justify-between items-center mb-6 no-print">
                    <button onClick={() => setRoute(AppRoute.DASHBOARD)} className="text-gray-500 hover:text-black flex items-center gap-1">
                    ...
                    >
                        <span className="material-icons">print</span> Imprimir / PDF
                    </button>
                </div>
