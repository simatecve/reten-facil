...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (session?.user) {
        fetchProfile(session.user.id);
        if (event === 'SIGNED_IN') setRoute((localStorage.getItem('last_route') as AppRoute) || AppRoute.DASHBOARD);
  ...
  const handleCreateCompany = async (e: React.FormEvent) => {
    e.preventDefault(); if (!userProfile || userProfile.role !== 'admin') return;
